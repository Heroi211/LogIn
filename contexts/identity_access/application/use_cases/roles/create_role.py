...
from contexts.identity_access.application.dto.roles import CreateRoleInput, RoleRecord
...
class CreateRole:
    ...
    async def execute(self, data: CreateRoleInput, audit_ctx: AuditContext | None = None) -> RoleRecord:
        role = Role.create(description=data.description, active=data.active)
        saved = await self._roles.add(role)
        await record_audit(
            self._audit,
            audit_ctx,
